...
/// Purpose: Owns paging through the Business Central payments endpoint for
/// one caller's lifetime, following the same lifecycle/error-handling
/// architecture as `LedgerEntriesService`. Not yet wired to any screen —
/// this is the reusable data-load engine only.
///
/// Responsibilities:
...
///   the fixed payments endpoint with the original `perPage` — never by
///   following a backend-provided `next_page_url`/`first_page_url`/
///   `last_page_url`/`prev_page_url`/`path`, since those have been observed
///   to drop the endpoint path for this API. Stops once
///   `currentPage >= lastPage`, per the backend's reported page numbers.
...
/// - Deduplicate rows by `entryNo` when appending a page, preserving the
///   backend's row order.
...
/// - Sort rows, derive a payment status, or compute `amount -
///   remainingAmount` — this class is transport/pagination only.
class PaymentsService {
  ...
  List<PaymentEntry> _entries = const [];
  int _currentPage = 1;
  int _lastPage = 1;
  ...
  List<PaymentEntry> get entries => _entries;
  ...
  List<PaymentEntry> _deduplicated(
    List<PaymentEntry> incoming, {
    required List<PaymentEntry> against,
  }) {
