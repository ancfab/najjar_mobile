// Unit tests for ItemsService: first-page load, load-more append via
// currentPage + 1 (never a next_page_url — the confirmed live envelope
// returns the path-dropping "/?page=2"), fixed perPage preservation,
...
// generation guard, id-based dedupe (never commonItemNo, since multiple
// item variants share it), 401 handoff to the session coordinator, and the
// items-specific 422 mapping (never BusinessCentralAccountNotLinked, since
// this catalog is company-scoped rather than tied to a linked customer).
//
...
import 'package:anc_fabrics/services/items_service.dart';
...
Map<String, dynamic> _itemJson(
  ...
  String commonItemNo = 'COMMON-001',
}) => {
  'id': id,
  'itemNo': itemNo,
...
/// items response: a bare `/?page=2` that drops the endpoint path) to
/// prove ItemsService never reads or follows them.
Map<String, dynamic> _envelope({
  ...
  'data': [for (final id in ids) _itemJson(id)],
...
void main() {
  group('ItemsService.loadFirstPage', () {
    test('loads the first page correctly', () async {
      ...
      expect(service.items.map((e) => e.id), ['id-1', 'id-2']);
      expect(fakeHttp.requestCount, 1);
    });

    test('requests the items endpoint with page=1 and per_page=25', () async {
      final fakeHttp = _ScriptedHttpClient([
        (req) async =>
      ...
      expect(fakeHttp.requestedUrls.single.path, '/api/business-central/items');
  ...
  group('ItemsService.loadNextPage', () {
    test('requests page = currentPage + 1 against the fixed items endpoint, '
      ...
      // Both requests must hit the fixed items path with page/per_page
    ...
    test('a load-more failure preserves existing rows', () async {
      ...
      expect(service.items.map((e) => e.id), ['id-1']);
    });
    ...
    test('preserves backend row order across pages', () async {
      ...
      expect(
        ...
        reason: 'ItemsService must not sort rows locally.',
      );
    });
    ...
    test('duplicate id rows across pages are not duplicated', () async {
      ...
      expect(service.items.map((e) => e.id), ['id-1', 'id-2', 'id-3']);
    });

    test('items sharing the same commonItemNo (different variants) are both '
        'kept — deduplication is by id only, never commonItemNo', () async {
      final variantA = _itemJson('id-1', commonItemNo: 'COMMON-001');
      final variantB = _itemJson('id-2', commonItemNo: 'COMMON-001');
      final fakeHttp = _ScriptedHttpClient([
      ...
      expect(service.items.map((e) => e.id), ['id-1', 'id-2']);
      expect(service.items.map((e) => e.commonItemNo).toSet(), {'COMMON-001'});
    });
  });

  group('ItemsService.refresh', () {
    test('resets pagination and replaces items', () async {
      final fakeHttp = _ScriptedHttpClient([
        (req) async => _jsonResponse(
          200,
          _envelope(ids: ['id-1'], lastPage: 2),
      ...
      expect(service.items.map((e) => e.id), ['id-99']);
      expect(service.hasNextPage, isFalse);
  ...
  group('ItemsService session/failure handling', () {
    ...
    test('a 422 with no page/per_page error still maps to '
        'BusinessCentralRequestDefect for items, never '
        'BusinessCentralAccountNotLinked — this catalog is company-scoped, '
